...
describe("ExportCommand", () => {
  ...
  test("resolves a relative --out against the caller's cwd rather than the project root", async () => {
    const project = join(tmpRoot, "repo");
    ...
    await new ExportCommand().execute(ctx({ cwd: nested, out: "out.env" }));

    expect(readFileSync(join(nested, "out.env"), "utf8")).toBe("APP=web\n");
