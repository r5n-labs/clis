...
`);
  ...
  test("matches Bun for a double-quoted value spanning two lines", () => {
    const input = 'A="first\nsecond"\nB=b\n';
  ...
  test("matches Bun for a multiline value containing a KEY=VALUE-shaped inner line", () => {
    const input = 'A="first\nB=second"\n';
    const expected = { A: "first\nB=second" };
  ...
  test("matches Bun for an unterminated double quote at EOF", () => {
    const input = 'A="first\nB=second\n';
  ...
  test("keeps single-line unterminated double quotes literal", () => {
    const input = 'A="first\n';
    const expected = { A: '"first' };
...
describe("serializeDotenv", () => {
  ...
  test("rejects NUL and values outside the minimal representable grammar", () => {
    ...
    for (const value of [nulValue, mixedQuoteValue]) {
      ...
      expect(message).toBe("Unrepresentable dotenv value for key SECRET");
