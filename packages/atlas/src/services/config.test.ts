...
describe("loadAtlasConfig", () => {
  ...
  const malformedConfigCases: ReadonlyArray<readonly [string, unknown, string]> = [
    ...
    ["top-level unknown key", { profiles: {}, secret: { vars: {} } }, 'config has unknown key "secret"'],
    [
      "defaults unknown key",
      { defaults: { exportfile: ".env" }, profiles: {} },
      'defaults has unknown key "exportfile"',
    ],
    [
      "profile unknown key",
      { profiles: { web: { envfiles: [".env"] } } },
      'profiles["web"] has unknown key "envfiles"',
    ],
    [
      "secret ref unknown key",
      { profiles: { web: { secrets: { TOKEN: { env: "SOURCE", required: true } } } } },
      'profiles["web"].secrets["TOKEN"] has unknown key "required"',
    ],
  ];
