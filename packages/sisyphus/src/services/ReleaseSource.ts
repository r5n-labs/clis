...
export async function hashReleaseSource(sisyphusDir: string): Promise<string> {
  ...
  if (!metadataRelativePath || isEscapingPath(metadataRelativePath)) {
...
export async function validateReleaseCheckout(cwd: string = process.cwd()): Promise<void> {
  const rootResult = await Bun.$`git rev-parse --show-toplevel`.cwd(cwd).quiet();
  const repositoryRoot = resolve(rootResult.stdout.toString().trim());
  await validateRepositoryCheckout(repositoryRoot);
}

async function validateRepositoryCheckout(repositoryRoot: string): Promise<void> {
  ...
  for (const record of splitNullTerminated(flagsResult.stdout)) {
  ...
  for (const record of splitNullTerminated(indexResult.stdout)) {
    ...
    await validateRepositoryCheckout(absolutePath);
  }
}

export function hashReleasePlan(plan: ReleasePlan, stones: readonly StoneJson[]): string {
  return createHash("sha256")
    .update(JSON.stringify(canonicalizeJson({ ...plan, stones })))
    .digest("hex");
}
...
export function isEscapingPath(path: string): boolean {
...
async function listSourceEntries(repositoryRoot: string, metadataPath: string): Promise<SourceEntry[]> {
  ...
  for (const path of splitNullTerminated(untrackedResult.stdout)) {
...
function splitNullTerminated(output: Buffer): string[] {
  ...
  if (!Buffer.from(text).equals(output)) {
    throw new Exit("Release source contains paths that are not valid UTF-8");
  }
...
async function resolveSourcePath(repositoryRoot: string, path: string): Promise<string> {
  ...
  if (
    ...
    isEscapingPath(repositoryRelativePath) ||
...
async function resolveGitlinkObjectId(
  ...
  if (
    ...
    !moduleRelativePath ||
    isEscapingPath(moduleRelativePath)
  ) {
    throw new Exit(`Release source gitlink is not initialized: ${entry.path}`);
  }
