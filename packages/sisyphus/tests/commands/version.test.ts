...
describe("VersionCommand non-interactive flags", () => {
  ...
  test("headless flag-only invocation without --yes creates a stone when stdout is not a TTY", async () => {
    const cliPath = join(import.meta.dir, "../../src/cli.ts");
    const subprocess = Bun.spawn(
      [process.execPath, cliPath, "version", "--all", "--bump", "patch", "--message", "release: headless"],
      { cwd: root, stderr: "pipe", stdin: "ignore", stdout: "pipe" },
    );
    const [stdout, stderr, exitCode] = await Promise.all([
      new Response(subprocess.stdout).text(),
    ...
    expect(exitCode).toBe(0);
    expect(`${stdout}\n${stderr}`).toContain("created");
    const [stone] = readStones();
    expect(stone?.message).toBe("release: headless");
    expect([...(stone?.patch ?? [])].sort()).toEqual(["@fixture/bar", "@fixture/foo"]);
  });
  ...
  test("--from-commits with a recorded baseline reports no conventional commits instead of failing", async () => {
    ...
    config.set("lastStone", { commit: baseCommit, date: "2026-07-28" });

    const ctx = makeCtx(config, { fromCommits: true, yes: true });
